...
const RELEVANT_SECTION_PATTERNS = [
  ...
  /ЕДИНЫЙ\s+ПЛАТЕ[Ж]/i,
  // Имущественные налоги ИП/физлица: транспорт, земля, имущество — частые
  // вопросы владельцев авто/недвижимости (небольшие, чётко очерченные разделы).
  /НАЛОГ\s+НА\s+ТРАНСПОРТ/i,
  /ЗЕМЕЛЬНЫЙ\s+НАЛОГ/i,
  /НАЛОГ\s+НА\s+ИМУЩЕСТВО/i,
];
